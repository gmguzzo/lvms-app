...
export class LayoutService {
  ...
  applyMatTheme() {
    this.themeService.applyMatTheme(this.getLayoutConfigMatTheme());
  }

  getLayoutConfigMatTheme() {
    if (this.layoutConf && this.layoutConf.matTheme) {
      return this.layoutConf.matTheme;
    }
    return '';
  }
  ...
  setLayoutFromQuery() {
    ...
    try {
      ...
      this.themeService.changeTheme(prevTheme, this.getLayoutConfigMatTheme());
    } catch (e) {
    }
  }
  ...
  adjustLayout(options: IAdjustScreenOptions = {}) {
    ...
    if (this.currentRoute) {
      this.fullWidthRoutes.forEach((route) => {
        if (this.currentRoute && this.currentRoute.indexOf(route) !== -1) {
          sidebarStyle = 'closed';
        }
      });
    }

    this.publishLayoutChange({ isMobile: this.isMobile });
  }
